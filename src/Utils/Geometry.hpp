...
#ifndef GP__GEOMETRY_HPP_
#define GP__GEOMETRY_HPP_
...
namespace GP
{
    ...
    static inline
    void AABoxAsPolygon2D(const double minx,
			  const double miny,
			  const double maxx,
			  const double maxy,
			  double       poly[8])
    {
	poly[0] = minx; 	poly[1] = miny; 
	poly[2] = maxx; 	poly[3] = miny; 
	poly[4] = maxx; 	poly[5] = maxy;
	poly[6] = minx; 	poly[7] = maxy;
    }
    ...
    void ApplyTransRotToPolygon2D(const double TR[],
       ...
       void StabSkeleton2D(const int n,
			const double skeleton[],
			const double dist,
			std::vector<double> * const newsk);

    bool ShiftSkeleton2D(const int n,
		       const double skel[],
		       const double thick,
    ...
    bool FromSkeletonToPolygon2D(const int n, 
				 const double skeleton[],
				 const double thick,
				 std::vector<double> * const poly);
