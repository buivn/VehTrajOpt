...
#ifndef GP__TRIMESH_HPP_
#define GP__TRIMESH_HPP_
...
namespace GP
{
	...
	static const int NR_SLICES;
	static const int NR_STACKS;
	
	virtual void Clear(void);
	
	int GetNrVertices(void) const
	{
	    return m_vertices.size();
	}
	const double* GetVertex(const int i) const
	{
	    return m_vertices[i]->m_pos;		
	}
	const double* GetVertexNormal(const int i) 
	{
	...
	int GetNrTexCoords(void) const
	{
	    return m_texCoords.size();
	}

	TexCoord GetTexCoord(const int i) const
	{
	...
	int GetNrFaces(void) const
	{
	    return m_faces.size();
	}
	
	const Face* GetFace(const int i) 
	{
	...
	int GetNrGroups(void) const
	{
	    return m_groups.size();
	}
	const Group* GetGroup(const int i) const
	{
	    return m_groups[i];
	}
	...
	int GetNrMaterials(void) const
	{
	    return m_materials.size();
	}
	const GMaterial* GetMaterial(const int i) const
	{
	    return m_materials[i];
	}
	GMaterial* GetMaterial(const int i)
	{
	    return m_materials[i];
	}
	int GetCurrMaterialId(void) const
	{
	    return m_midCurr;
	}

	int GetNrTextures(void) const
	{
	    return m_textures.size();
	}
	const GTexture* GetTexture(const int i) const
	{
	    return m_textures[i];
	}
	GTexture* GetTexture(const int i)
	{
	    return m_textures[i];
	}
	int GetCurrTextureId(void) const
	{
	    return m_tidCurr;
	}
	...
	virtual void AddTriMesh(TriMesh * const tmesh);
	...
	virtual void AddPolygon(Polygon2D * const poly);

		
	virtual void AddExtrudedPolygon(Polygon2D * const poly, 
	...
	const GTexture* GetMainTexture(void) const
	{
	    return m_gMainTexture;
	}
	
	GTexture* GetMainTexture(void) 
	{
	    return m_gMainTexture;
	}
	

	const GMaterial* GetMainMaterial(void) const
	{
	    return m_gMainMaterial;
	}
	
	GMaterial* GetMainMaterial(void) 
	{
	    return m_gMainMaterial;
	}
	
	void SetMainTexture(GTexture *gTex)
	{
	    m_gMainTexture = gTex;
	}

	void SetMainMaterial(GMaterial *gMat)
	{
	    m_gMainMaterial = gMat;
	}
	

	void ReadOgreMesh(const char meshName[], const char gtexName[]);
	

	void GenerateRandomTextureCoords(void);
	
	virtual void Draw(void);	    

	void *m_usrPtr;

	void GetFacesInsideSphere(const double center[3], const double r, std::vector<int> * fids);
	 
	int ClosestVertex(const double p[2]);
