...
#ifndef GP__Radix_HPP_
#define GP__Radix_HPP_
...
namespace GP
{
    ...
    typedef  unsigned long long RadixValType;
...
public:
  ...
  RadixValType weight;
  Link* next;
  Link(int n, RadixValType w, Link* p) {
    next = p;
    succid = n;
    weight = w;
  }
...
public: 
  ...
  Label state; 
  Link* edges; 
  RadixValType element;
  int bucket;
  Node* pred;
  Node* succ;

    Node *m_parent;
    RadixValType m_parentCost;
...
public:
  RadixValType S;
  int B;
  RadixValType n; // number of nodes    
  Node** buckets;   
  RadixValType* u;   
  RadixValType* b;   

  Radix() {
    S = std::numeric_limits<RadixValType>::max();
    B = (int) (ceil(log(S) / log(2))) + 2;
    buckets = new Node* [B];
    b = new RadixValType [B];
    u = new RadixValType [B];
    for (int i = 0; i < B; i++) {
      buckets[i] = 0;
      b[i] = u[i] = 0;
    }
    b[0] = 1; 
    b[B-1] = std::numeric_limits<RadixValType>::max();
    for (int i = 1; i < B-1; i++)
      b[i] = 1L << (i-1);
    u[B-1] = std::numeric_limits<RadixValType>::max();
    n = 0;
  }
  ...
  void adjust(RadixValType m, int t) {
    int i;
    u[0] = m;
    for (i = 1; i < t; i++) {
      u[i] = u[i-1] + b[i];
  ...
  void decrease(Node* x, RadixValType k) {
...
public:
  ...
  RadixGraph(int n =0) {
    ...
    void addEdge(int i, int j, RadixValType w)
    {
  ...
  Node* randomNode() {
    ...
    RadixValType path(Node *s, std::vector<int> &ids)
    {
	RadixValType cost = 0;
