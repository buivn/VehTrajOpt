...
#ifndef GP__GRAPH_SEARCH_HPP_
#define GP__GRAPH_SEARCH_HPP_
...
namespace GP
{
	...
	void SetInfo(GraphSearchInfo<Key> *info)
	{
	    m_info = info;
	}
		
	bool DFS(const Key start, Key * const goal);
	
	bool BFS(const Key start, Key * const goal);
	...
	Key GetParent(const Key u) const
	{
	    auto curr = m_map.find(u);
	    if(curr == m_map.end())
		return u;
	    return curr->second.m_parent;
    ...
    template <typename Key>
    void GraphSearch<Key>::GetPathFromStart(const Key u, std::vector<Key> * const path) const
    {
    ...
    template <typename Key>
    void GraphSearch<Key>::GetReversePathFromStart(const Key u, std::vector<Key> * const rpath) const
    {
	rpath->clear();
    ...
    template <typename Key>
    bool GraphSearch<Key>::DFS(const Key start, Key * const goal)
    {	
    ...
    template <typename Key>
    bool GraphSearch<Key>::BFS(const Key start, Key * const goal)
    {	
	std::vector<Key>    edges;
	std::vector<int>    perm;
	std::vector<double> costs;
