...
#ifndef GP__ALGEBRA_HPP_
#define GP__ALGEBRA_HPP_
...
namespace GP
{
	...
	static inline double PointDistSquared(const int n, const double p1[], const double p2[])
	{
	    double d = 0;
	    for(int i = 0; i < n; ++i)
	...
	static inline double PointDist(const int n, const double p1[], const double p2[])
	{
	    return sqrt(PointDistSquared(n, p1, p2));
	}	    
	...
	{
	    return 
		fabs(VecDotProduct(n, v1, v2) / (VecNorm(n, v1) * VecNorm(n, v2)) - 1) 
		<= Constants::EPSILON;
	}	    
	...
	static inline int VecAlignment(const int n, const double v1[], const double v2[])
	{
	    const double c = VecDotProduct(n, v1, v2) / (VecNorm(n, v1) * VecNorm(n, v2));
	    return 
		fabs(c - 1) <= Constants::EPSILON ? 
		Constants::PARALLEL : (fabs(c + 1) <= Constants::EPSILON ? 
	...
	static inline void VecPrint(const int n, const double v[], FILE * const out = stdout)
	{
	    for(int i = 0; i < n; ++i)
		fprintf(out, "%f ", v[i]);
	}
	
	static inline void MatPrint(const int nrRows, const int nrCols, 
				    const double A[], FILE * const out = stdout)
	{
	    for(int i = 0; i < nrRows; ++i)
	    {
		VecPrint(nrCols, MatGetRow(nrRows, nrCols, A, i), out);
		fprintf(out, "\n");
	    }
	}
