...
namespace GP
{
	...
	GDrawData(void)
	{
	    ...
	    m_params[GDRAW_FONT_SIZE_X] = 1;
	    m_params[GDRAW_FONT_SIZE_Y] = 1;
	    m_params[GDRAW_FONT_SIZE_Z] = 2;	    
    ...
    void GDrawSetupFromParams(Params * const p)
    {
	GDrawData *d = GDrawGetData();
	
	d->m_params[GDRAW_NR_STACKS]   = p->GetValueAsDouble("GraphicsNrStacks",  d->m_params[GDRAW_NR_STACKS]);
	d->m_params[GDRAW_NR_SLICES]   = p->GetValueAsDouble("GraphicsNrSlices",  d->m_params[GDRAW_NR_SLICES]);
	d->m_params[GDRAW_FONT_SIZE_X] = p->GetValueAsDouble("GraphicsFontSizeX", d->m_params[GDRAW_FONT_SIZE_X]);
	d->m_params[GDRAW_FONT_SIZE_Y] = p->GetValueAsDouble("GraphicsFontSizeY", d->m_params[GDRAW_FONT_SIZE_Y]);
	d->m_params[GDRAW_FONT_SIZE_Z] = p->GetValueAsDouble("GraphicsFontSizeZ", d->m_params[GDRAW_FONT_SIZE_Z]);
     }
...
//general	    
    ...
    void GDrawIndexColor(const int i, double *rgb)
    {
	const double reds[]   = {0.0, 0.9, 0.3, 0.6, 0.2, 0.7};
	const double greens[] = {0.0, 0.6, 0.9, 0.3, 0.4, 0.5};
	const double blues[]  = {0.0, 0.6, 0.3, 0.9, 0.8, 0.9};
	const int    n        = 6;

	const double r = reds[i % n];
	const double g = greens[(i/n) % n];
	const double b = blues[(i / (n * n)) % n];
    ...
    void GDrawIndexColor(const int i, const int n, double rgb[])
    {
	double saturation=1.0;
	double value=1.0;
	double stepHue=360.0/n;
	
	double  hue      =  i * stepHue;
	double hueSixty = hue/60.0;
	double X        = (saturation*value) * (1.0-fabs(fmod(hueSixty,2.0)-1.0));
	double r, g, b;
...
//2D stuff	    
    ...
    void GDrawPolygon2D(Polygon2D * const poly)
    {
	if(poly->IsConvex())
	{
    ...
    void GDrawIllumination(const GIllumination * const gillum)
    {
	...
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, gillum->GetProperty(GIllumination::AMBIENT));
	glLightModeli (GL_LIGHT_MODEL_LOCAL_VIEWER, GL_TRUE);

	for(int i = 0; i < n && i < 8; ++i)
	{
	    const GLight *light = gillum->GetLight(i);
	    glLightfv(GL_LIGHT0 + i, GL_AMBIENT,  light->GetProperty(GLight::AMBIENT));
	    glLightfv(GL_LIGHT0 + i, GL_DIFFUSE,  light->GetProperty(GLight::DIFFUSE));
	    glLightfv(GL_LIGHT0 + i, GL_SPECULAR, light->GetProperty(GLight::SPECULAR));
	    glLightfv(GL_LIGHT0 + i, GL_POSITION, light->GetProperty(GLight::POSITION));
